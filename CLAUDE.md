...
- **No low-value tests.** A test earns its place when it can fail for a reason other than an edit to the text it asserts on. An assertion that a doc comment, a `docs/` page, or the specification holds a given phrase restates that text in a second file, so it fails when an author rewords the page and passes when the page is wrong. Test a documentation surface by compiling its `ts` blocks against the shipped types (`test/spec-guard.js`), by resolving the paths it names (`test/references.test.js`), or by deriving the expectation from the code it documents. Where nothing derives, the documentation stands on review, and no test is added.
  ...
  A mutation test that edits a page's text checks the edit against its own input — `assert.notEqual(mutated, page)` — before it runs the guard. When the page is reworded out from under the mutation, that check names the stale mutation, where the guard's own assertion would blame the guard.

- **Control, not check.** When one fact is written in more than one place, delete the copies or derive them from the one that stays. Do not add a test asserting the copies agree: that leaves every degree of freedom standing and adds one more moving part (constitution, Arts. 2 and 3). When an issue asks for such a test, say so before building it, and ask which copy is the source of truth.

- **No volatile specifics in prose.** A spec, doc, issue or pull request body names where a value is declared rather than restating it: a dependency version lives in `package.json`, and a test result is "green from a clean checkout", not a count. A restated value is true when written and goes stale silently when another change lands. Where a number genuinely helps, pin it to something that moves with it — a commit or a run.
